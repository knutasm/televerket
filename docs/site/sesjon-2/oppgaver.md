---
title: Sesjon 2 — Oppgaver
description: Oppgaver for sesjon 2 om sources, staging-modeller og testing
---
...
Merk:
- Hvorfor skal `stg_catalog__products` bruke `{{ ref('catalog_products') }}` og ikke `{{ source() }}`?
...
Krav:
- Dedupliser `stg_billing__payments` med `ROW_NUMBER()` og behold første betaling per faktura
